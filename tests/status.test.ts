...
test("server instances identify discoverable TUI sockets without command events", () => {
  ...
  const socketPath = productivityTuiSocketPath(directory, process.pid, `discovery-${Date.now()}`)
  assert.equal(isProductivityTuiSocketName(directory, process.pid, path.basename(socketPath)), true)
  assert.equal(isProductivityTuiSocketName(directory, process.pid + 1, path.basename(socketPath)), false)
  assert.equal(isProductivityTuiSocketName(`${directory}-other`, process.pid, path.basename(socketPath)), false)
  assert.equal(isProductivityTuiSocketName(directory, process.pid, `${path.basename(socketPath)}.stale`), false)
})
