...
function createPexpectHistoryScript(): { file: string; dispose: () => void } {
  ...
  writeFileSync(file, [
    ...
    "    child.sendcontrol('p')",
    "    child.expect_exact('Commands', timeout=20)",
    "    captured.append(str(child.before) + 'Commands')",
    "    child.send('Search Prompt History')",
    "    child.expect_exact('Search Prompt History', timeout=20)",
    "    captured.append(str(child.before) + 'Search Prompt History')",
    "    child.send('\\r')",
